...
class TestRunner:
    ...
    def check_scripts_exist(self):
        ...
        if missing_scripts:
            self.failure_reason = (
                f"Отсутствуют скрипты: {', '.join(missing_scripts)}"
            )
    ...
    def compile_code(self):
        ...
        result = subprocess.run(
            ["./compile_server.sh"], capture_output=True, text=True
        )
        if result.returncode != 0:
            ...
            self.failure_reason = f"Ошибка компиляции сервера: {error_msg}"
            self.error(self.failure_reason)
            return False

        self.info("Компиляция клиентского кода...")
        result = subprocess.run(
            ["./compile_client.sh"], capture_output=True, text=True
        )
        if result.returncode != 0:
            self.compilation_failed = True
            ...
            self.failure_reason = f"Ошибка компиляции клиента: {error_msg}"
            self.error(self.failure_reason)
            return False
    ...
    def start_server(self):
        try:
            ...
            if self.server_process.poll() is not None:
                ...
                self.failure_reason = f"Сервер завершился с ошибкой: {error}"
                self.error(self.failure_reason)
                return False
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                if response == "OK\n":
                    ...
                    self.error(
                        f"Неверный ответ сервера. Ожидался 'OK\\n', "
                        f"получен: {repr(response)}"
                    )
                    return False
    ...
    def test_multiple_connections(self):
        ...
        if success_count == total_tests:
            ...
            self.error(
                f"Успешно только {success_count} из {total_tests} подключений"
            )
            return False
    ...
    def stop_server(self):
        if self.server_process:
            try:
                if os.name != "nt":
                    os.killpg(
                        os.getpgid(self.server_process.pid), signal.SIGTERM
                    )
                else:
